# @japanese-tts-analyzer/analyze-backend
...
```bash
pnpm --filter @japanese-tts-analyzer/analyze-backend build
...
```bash
docker run --rm -p 8789:8080 japanese-tts-analyzer-analyze-backend-local
```
...
- Cloudflare Containers の local dev を試す場合は `pnpm --filter @japanese-tts-analyzer/analyze-backend preview` でも起動できますが、Docker 直起動の方が切り分けしやすいです。
