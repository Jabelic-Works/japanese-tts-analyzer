...
const isQuotativeVerb = (token?: UniDicRawToken): token is UniDicRawToken =>
  Boolean(token && (isThinkingVerb(token) || isSayingVerb(token)));
...
export const matchQuotativeTeIruExpression = (
  tokens: readonly UniDicRawToken[],
  index: number
): TokenOverrideMatch | undefined => {
  const quoteToken = tokens[index];
  const verbToken = tokens[index + 1];
  const connectiveToken = tokens[index + 2];
  const iruToken = tokens[index + 3];

  if (
    !quoteToken ||
    quoteToken.partOfSpeech.level1 !== "助詞" ||
    quoteToken.surface !== "と" ||
    !isQuotativeVerb(verbToken) ||
    !isConnectiveParticle(connectiveToken) ||
    !isIruVerb(iruToken)
  ) {
    return undefined;
  ...
  const sourceTokens = [quoteToken, verbToken, connectiveToken, iruToken];
